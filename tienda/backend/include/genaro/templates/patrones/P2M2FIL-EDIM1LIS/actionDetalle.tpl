...
class <<$classname_detalle|capitalize>> extends gvHidraForm_DB
{
	public function __construct() {
		...
		$str_select = "SELECT <<section name=select loop=$fields_detalle>><<$fields_detalle[select]>> as \"lis_<<$fields_detalle[select]>>\"<<if $smarty.section.select.last>><<else>>, <</if>><</section>> FROM <<$tablename_detalle>>";
...
<<if $componente eq 4>>
<<*Lista*>>
		$lista_lis = new gvHidraList('lis_<<$fields_detalle[components]>>');
