...
class <<$classname_maestro|capitalize>> extends gvHidraForm_DB
{
	public function __construct() {
		...
		$str_select = "SELECT <<section name=select loop=$fields_maestro>><<$fields_maestro[select]>> as \"edi_<<$fields_maestro[select]>>\"<<if $smarty.section.select.last>><<else>>, <</if>><</section>> FROM <<$tablename_maestro>>";
...
<<section name=edi loop=$fields_maestro>>
		$this->addMatching("edi_<<$fields_maestro[edi]>>","<<$fields_maestro[edi]>>","<<$tablename_maestro>>");
<</section>>
...
<</section>>
		...
		$this->addSlave('<<$classname_detalle|capitalize>>', array(<<section name=pk_maestro loop=$primaryKeyMaestroArray>>'edi_<<$primaryKeyMaestroArray[pk_maestro]>>',<</section>>), array(<<section name=fk_detalle loop=$foreignKeyDetalleArray>>'lis_<<$foreignKeyDetalleArray[fk_detalle]>>',<</section>>));
