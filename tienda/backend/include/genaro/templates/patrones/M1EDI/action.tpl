...
class <<$classname_detalle|capitalize>> extends gvHidraForm_DB
{
	public function __construct() 	{
		...
		$str_select = "SELECT <<section name=select loop=$fields_detalle>><<$fields_detalle[select]>> as \"<<if $foreignKey_detalle eq "true">>$fields_detalle[select]<<else>>edi_<<$fields_detalle[select]>><</if>>\"<<if $smarty.section.select.last>><<else>>, <</if>><</section>> FROM <<$tablename_detalle>>";
...
<<section name=edi loop=$fields_detalle>>
		$this->addMatching("<<if $foreignKey_detalle[edi] eq "true">><<$fields_detalle[edi]>><<else>>edi_<<$fields_detalle[edi]>><</if>>","<<$fields_detalle[edi]>>","<<$tablename_detalle>>");
<</section>>
...
<<if $componente eq 4>>
<<*Lista*>>
		$lista_edi = new gvHidraList('edi_<<$fields[components]>>');
