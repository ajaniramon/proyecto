...
class <<$classname_maestro|capitalize>> extends gvHidraForm_DB
{
	public function __construct() {
		...
		$str_select = "SELECT <<section name=select loop=$fields_maestro>><<$fields_maestro[select]>> as \"<<if $primaryKey_maestro[select] eq "true">><<$fields_maestro[select]>><<else>>lis_<<$fields_maestro[select]>><</if>>\"<<if $smarty.section.select.last>><<else>>, <</if>><</section>> FROM <<$tablename_maestro>>";
...
<<section name=lis loop=$fields_maestro>>
		$this->addMatching("<<if $primaryKey_maestro[lis] eq "true">><<$fields_maestro[lis]>><<else>>lis_<<$fields_maestro[lis]>><</if>>", "<<$fields_maestro[lis]>>", "<<$tablename_maestro>>");
<</section>>
		/************************ END MATCHINGs ************************/
