...
class <<$classname_detalle|capitalize>> extends gvHidraForm_DB
{
	public function __construct() {
		...
		$str_select = "SELECT <<section name=select loop=$fields_detalle>><<$fields_detalle[select]>> as \"<<if $primaryKeyDetalle eq "true">>edi_id_detalle<<elseif $fields_detalle[select] eq "$foreignKeyDetalle">><<$tipoMaestro>>_id_maestro<<else>>edi_<<$fields_detalle[select]>><</if>>\"<<if $smarty.section.select.last>><<else>>, <</if>><</section>> FROM <<$tablename_detalle>>";		 
...
<<section name=lis loop=$fields_detalle>>
		...
		//$this->addMatching("<<if $primaryKey_detalle[matchings] eq "true">>edi_id_detalle<<elseif $fields_detalle[matchings] eq "$foreignKeyDetalle">><<$tipoMaestro>>_id_maestro<<else>>edi_<<$fields_detalle[matchings]>><</if>>","<<$fields_detalle[matchings]>>","<<$tablename_detalle>>");
<</section>>
