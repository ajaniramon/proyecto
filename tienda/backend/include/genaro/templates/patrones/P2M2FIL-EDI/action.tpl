...
class <<$classname_maestro|capitalize>> extends gvHidraForm_DB
{
	public function __construct() {
		...
		$str_select = "SELECT <<section name=select loop=$fields_maestro>><<$fields_maestro[select]>> as \"<<if $primaryKey_maestro[select] eq "true">><<$fields_maestro[select]>><<else>>edi_<<$fields_maestro[select]>><</if>>\"<<if $smarty.section.select.last>><<else>>, <</if>><</section>> FROM <<$tablename_maestro>>";
...
<<section name=fil loop=$fields_maestro>>
		$this->addMatching("<<if $primaryKey_maestro[fil] eq "true">><<$fields_maestro[fil]>><<else>>fil_<<$fields_maestro[fil]>><</if>>","<<$fields_maestro[fil]>>","<<$tablename_maestro>>");
<</section>>

		//Modo de trabajo EDI
<<section name=edi loop=$fields_maestro>>
		$this->addMatching("<<if $primaryKey_maestro[edi] eq "true">><<$fields_maestro[edi]>><<else>>edi_<<$fields_maestro[edi]>><</if>>","<<$fields_maestro[edi]>>","<<$tablename_maestro>>");
<</section>>
...
<</if>>
		$this->addFieldType('edi_<<$fields_maestro[fecha]>>',$fecha);
...
<</if>>
<</if>>
		$this->addFieldType('edi_<<$fields_maestro[string]>>',$string);
...
<</if>>
		$this->addFieldType('edi_<<$fields_maestro[int]>>',$int);
...
<</if>>
		$this->addFieldType('edi_<<$fields_maestro[float]>>',$float);
...
<<*CheckBox*>>
		...
		$check_edi = new gvHidraCheckBox('edi_<<$fields_maestro[components]>>');
...
<<*RadioButton*>>
		...
		$radio_edi = new gvHidraList('edi_<<$fields_maestro[components]>>');
...
<<*Lista*>>
		...
		$lista_edi = new gvHidraList('edi_<<$fields_maestro[components]>>');
...
<<foreach key=key item=item from=$array_claves_detalles>>
		$this->addSlave('<<$key|capitalize>>', array('<<$primaryKeyMaestro>>'), array('<<$item>>'));
<</foreach>>
