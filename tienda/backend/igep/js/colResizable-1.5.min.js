...
(function ($) {
	...
	h.append("<style type='text/css'>" +
			...
			".JColResizer td, " +
			".JColResizer th{overflow:hidden;padding-left:0!important; padding-right:0!important;}  " +
			".JCLRgrips{ height:0px; position:relative;} " +
			".JCLRgrip{margin-left:-5px; position:absolute; z-index:5; } " +
			".JCLRgrip " +
			".JColResizer{position:absolute;background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;height:100%;cursor: e-resize;top:0px} " +
			".JCLRLastGrip{position:absolute; width:1px; } " +
			".JCLRgripDrag{ border-left:1px dotted black;	} " +
			".JCLRFlex{width:auto!important;}</style>");
	
	
	/*El diseño se ​​cambia por primera vez cuando la tabla se le asigna la clase SIGNATURE que incluye table-layout : fixed ;
	 *  Justo antes de eso, se añaden los anchos de columna originales en una nueva matriz . Esta matriz se pasa a la función createGrips .*/	
...
createGrips(t, originalColumnWidths);
	...
	createGrips = function (t,originalColumnWidths) {
...
if (i==0)
{
	ubicacion = th.attr('data-gvhTypeTab');
	c.w = originalColumnWidths[i]; // No tiene columna check
// panelList
	if (ubicacion == 'panelList')
		c.w = '19px';
// panelTab
	if (ubicacion == 'panelTab')
		c.w = '32px';
// panelTabRowEdit
	if (ubicacion == 'panelTabRowEdit')
		c.w = '50px';
}
else
